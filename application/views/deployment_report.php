...
<md-content class="mylayout-padding">
	<div layout="column">
		<h3 class="heading-stylerespond">Deployment Report</h3>
		<div>
			<div layout ="row">
				<div layout ="row" flex="100">
					<canvas id="bar" class="chart chart-bar" height="80"                            chart-data="deployementgraphdata" chart-labels="deployementgraphlabels" chart-series="deployementgraphseries"></canvas>
				</div>
			</div>
			<form name="Search" method="post">
				<div flex layout-gt-sm="row"  layout="row">
					...
					<div flex="5" ng-show="user_role_code==HMADMIN" hide-xs hide-sm></div>
					<md-button  ng-show="Deployment_pdf_report==Generate PDF" class="md-button md-raised md-accent" ng-click="getDeployementReportPDF(deployemnt_reports)" aria-label="submit">Export</md-button>
				</div>
			</form>
		</div>
			...
			<table class="md-api-table table table-bordered" style="width:100%;">
				...
				<tbody ng-if="!isEmpty(deployemnt_reports)">
					<tr ng-repeat="deployemnt_report in deployemnt_reports">
						...
						<td>
							<button ng-if="user_role_code!=HMADMIN"  ng-show="Deployment_pdf_report==Generate PDF" ng-click="pdfDeploymentReportTCPDF($event,deployemnt_report)" class="btn btn-xs btn-default" aria-label="Edit">
