...
<md-content class="mylayout-padding">
	<div layout="column">
		<h3 class="heading-stylerespond">Calibration Reports</h3>
		<div layout ="row">
			<div layout ="row" flex="100">
				<canvas id="pie" class="chart chart-bar" height="50"                        chart-data="qcgraphdata" chart-labels="qcgraphlabels" chart-series="qcgraphseries"></canvas>
			</div>
		</div>
		<form name="Search" method="post">
			<div flex layout-gt-sm="row"  layout="row">
				...
				<md-input-container class="md-block" flex-gt-sm flex="10">
					...
					<input type="search" ng-model="qc_report_search.saccessoriesno" name="saccessoriesno" aria-label="saccessoriesno"/>
				</md-input-container>
				<div flex="5" hide-xs hide-sm>&nbsp;&nbsp;(OR)</div>
				<md-input-container class="md-block"  flex-gt-sm flex="15">
					<label>Eq. Name</label>
					<input type="text" ng-model="qc_report_search.ename" name="ename" aria-label="ename"/>
				</md-input-container>
				<div flex="5" hide-xs hide-sm>&nbsp;&nbsp;(OR)</div>
				<!---<md-input-container flex="20"><label>Select Department</label><md-select  ng-model="qc_report_search.dept_id" name="depts"><md-option ng-value="all">All</md-option><md-option ng-repeat="dept in depts"  ng-value="dept.CODE">                                {{dept.USER_DEPT_NAME}} ({{dept.CODE}})                            </md-option></md-select></md-input-container>--->
				<md-autocomplete flex="20" class="md-block" flex-gt-sm                             ng-value="qc_report_search.dept_id=searched.CODE"                             md-no-cache="false"                             md-selected-item="searched.CODE"                             md-search-text="searchDepartment"                             md-items="item in searchTextChange(searchDepartment,'Department')"                             md-item-text="item.USER_DEPT_NAME"                             md-min-length="0"                             md-floating-label="Department">
					<md-item-template>
				...
				<span ng-value="qc_report_search.dept_id = searched.CODE.CODE"></span>
				...
				<mdp-date-picker mdp-placeholder="Due Date"  class="md-block" flex-gt-sm flex="15" mdp-format="DD-MM-YYYY" mdp-max-date="maxDate"  ng-model="qc_report_search.fromdate"></mdp-date-picker>
				<div flex="5" hide-xs hide-sm>
					<!-- Space -->
				</div>
				<md-input-container flex="15">
					<label>Cal. Type</label>
					<md-select required ng-model="qc_report_search.qc_type" name="qc_type">
				...
				<md-button type="submit" value="Submit" class="md-icon-button md-raised md-primary" ng-click="getQCReport()" aria-label="submit">
					<ng-md-icon icon="search" style="fill:#fff" size="24"></ng-md-icon>
					<div flex="10" hide-xs hide-sm></div>
				</md-button>
				<md-button ng-show="QC_pdf_report==Generate PDF" class="md-button md-raised md-accent" ng-click="getQCReportPDF(qc_report)" aria-label="submit">Export</md-button>
			</md-button>
		</div>
	</form>
</div>
<div layout-gt-sm="row" layout="row" >
	<table class="md-api-table table table-bordered" style="width:100%;">
		...
		<tbody ng-if="!isEmpty(qc_reports)">
			<tr ng-repeat="qc_report in qc_reports">
				<td>{{qc_report.EID}}</td>
				<td>{{qc_report.dept_id}}</td>
				<td>{{qc_report.equp_name}}</td>
				<td>{{qc_report.model}}</td>
				<td>{{qc_report.es_number}}</td>
				<td>{{qc_report.contract_type}}</td>
				<td>
					<button ng-if="user_role_code!=HMADMIN" ng-show="QC_pdf_report==Generate PDF" ng-click="pdfQCReportTCPDF($event,qc_report)" class="btn btn-xs btn-default" aria-label="Edit">
...
<div flex layout="row" class="marginb-10" >
	...
	<div flex-xs="100" flex="60" layout="column" layout-align="end end">
		<cl-paging flex cl-pages="paging.total" , cl-steps="5" , cl-page-changed="getQCReport(paging.current)" , cl-align="end end" , cl-current-page="paging.current"></cl-paging>
	</div>
</div></div></md-content>