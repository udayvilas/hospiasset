...
function DatePickerCtrl($scope, $mdDialog, $mdMedia, $timeout, currentDate, options) {
	...
	this.yearItems = {
        ...
        END: (self.maxDate ? self.maxDate.year() : 4000),
                            ...
                            '<md-dialog-content layout="row" layout-wrap>' +
                                '<div layout="column" layout-align="start center">' +
                                    '<md-toolbar layout-align="start start" flex class="mdp-datepicker-date-wrapper md-hue-1 md-primary" layout="column">' +
                                        '<span class="mdp-datepicker-year" ng-click="datepicker.showYear()" ng-class="{ \'active\': datepicker.selectingYear }">{{ datepicker.date.format(\'YYYY\') }} <i class="fa fa-angle-right fa-2" aria-hidden="true"></i></span>' +
    ...
    return  {
        ...
        link: {
            ...
            post: function(scope, element, attrs, ngModel, $transclude) {
                ...
                scope.type = scope.dateFormat ? "text" : "text";
    ...
    return  {
        ...
        template: function(element, attrs) {
            ...
            return '<div layout layout-align="start start">' +
                    ...
                    '</md-button>' +
                    '<md-input-container' + (noFloat ? ' md-no-float' : '') + ' md-is-error="isError()">' +
                        '<input ng-click="showPicker($event)" type="{{ ::type }}"' + (angular.isDefined(attrs.mdpDisabled) ? ' ng-disabled="disabled"' : '') + ' aria-label="' + placeholder + '" placeholder="' + placeholder + '"' + (openOnClick ? ' ng-click="showPicker($event)" ' : '') + ' />' +
                    '</md-input-container>' +
                '</div>';
        },
        scope: {
        ...
        link: function(scope, element, attrs, ngModel, $transclude) {
            ...
            scope.type = scope.timeFormat ? "text" : "text"
